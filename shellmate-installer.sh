...
install_config() {
    ...
    # Install bash script to /usr/local/bin
    print_info "Installing ShellMate bash script..."
    if [ ! -f "$SCRIPT_DIR/src/shellmate.sh" ]; then
        print_error "Source script not found: $SCRIPT_DIR/src/shellmate.sh"
        exit 1
    fi
...
# Auto-generated by shellmate-installer.sh on $(date)
...
shellmate() {
    ...
    if [[ "\$1" == "--help" ]] || [[ "\$1" == "-h" ]]; then
        # Pass help request directly to the bash script
        shellmate-sh --help
        return \$?
    fi
    
    local cmd
    # Get the command from the AI service using bash script directly
    cmd=\$(shellmate-sh --seamless "\$*" 2>/dev/null)
    local exit_code=\$?
...
uninstall_config() {
    ...
    print_success "ShellMate uninstalled successfully!"
